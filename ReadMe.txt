================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : PGM プロジェクトの概要
===============================================================================

この PGM アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、PGM アプリケーションを構成する各ファイルの内容の概要が含まれています。
...
PGM.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CPGMApp アプリケーション クラスを宣言します。

PGM.cpp
    これはメインのアプリケーション ソース ファイルで、CPGMApp アプリケーション クラスが含まれています。
...
res\PGM.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル PGM.rc にインクルードされます。
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、PGM.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////

アプリケーション ウィザードは 1 つのドキュメントの種類と 1 つのビューを作成します。

PGMDoc.h, PGMDoc.cpp - ドキュメント
    これらのファイルには、CPGMDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CPGMDoc::Serialize を使用)。
    ドキュメントには次の文字列が含まれます。
        ファイル拡張子:      pgm
        ファイルの種類の ID:        PGM.Document
        メイン フレームのキャプション:  PGM
        ドキュメントの種類の名前:       PGM
        フィルター名:         PGM Files (*.pgm)
        ファイルの新しい短い名前: PGM
        ファイルの種類の長い名前: PGM.Document

PGMView.h, PGMView.cpp - ドキュメントのビュー
    これらのファイルには、CPGMView クラスが含まれます。
    CPGMView オブジェクトは CPGMDoc オブジェクトを表示するために使用されます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、PGM.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。
